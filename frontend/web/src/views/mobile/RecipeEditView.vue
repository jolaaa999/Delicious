...
const ingredients = ref<Ingredient[]>([{ name: '', amount: 0, unit: 'g', note: '' }])
const processSteps = ref<ProcessStep[]>([{ order: 1, content: '' }])
...
async function loadRecipe() {
  ...
  try {
    ...
    if (ver) {
      ingredients.value = ver.ingredients?.length ? ver.ingredients.map((i) => ({ ...i })) : [{ name: '', amount: 0, unit: 'g' }]
      processSteps.value = ver.process_steps?.length ? ver.process_steps.map((s) => ({ ...s })) : [{ order: 1, content: '' }]
...
function addIngredient() { ingredients.value.push({ name: '', amount: 0, unit: 'g', note: '' }) }
function removeIngredient(idx: number) { if (ingredients.value.length > 1) ingredients.value.splice(idx, 1) }
function addStep() { processSteps.value.push({ order: processSteps.value.length + 1, content: '' }) }
function removeStep(idx: number) { if (processSteps.value.length > 1) { processSteps.value.splice(idx, 1); renumberSteps() } }
function renumberSteps() { processSteps.value.forEach((s, i) => { s.order = i + 1 }) }

function validate(): boolean {
  if (!name.value.trim()) return (errorMsg.value = '请填写菜名', false)
  if (ingredients.value.some((i) => !i.name.trim())) return (errorMsg.value = '请填写所有食材名称', false)
  if (processSteps.value.some((s) => !s.content.trim())) return (errorMsg.value = '请填写所有步骤内容', false)
  if (isEdit.value && !commitMsg.value.trim()) return (errorMsg.value = '编辑时请填写修改备注', false)
  errorMsg.value = ''
  return true
}
...
async function handleSubmit() {
  ...
  const payload = {
    ...
    ingredients: ingredients.value.map((i) => ({ name: i.name.trim(), amount: Number(i.amount) || 0, unit: i.unit.trim() || 'g', note: i.note?.trim() })),
    process_steps: processSteps.value.map((s, i) => ({ order: i + 1, content: s.content.trim(), duration_minutes: s.duration_minutes })),
  ...
  try {
    if (isEdit.value) {
      ...
      router.replace(`/m/kitchen/${recipeId.value}`)
    } else {
      const res = await createRecipe(payload)
      router.replace(`/m/kitchen/${res.recipe.id}`)
    }
  } catch (e: unknown) {
...
<template>
  <div class="edit-page">
    <header class="nav-bar">
      ...
      <button class="nav-bar__save" type="button" :disabled="saving || loading" @click="handleSubmit">{{ saving ? '保存中…' : '保存' }}</button>
    </header>
    ...
    <form v-else class="form" @submit.prevent="handleSubmit">
      ...
      <section class="form-section">
        <div class="section-head"><label class="field-label">食材与重量</label><button type="button" class="link-btn" @click="addIngredient">+ 添加</button></div>
      ...
      <section class="form-section">
        <div class="section-head"><label class="field-label">制作步骤</label><button type="button" class="link-btn" @click="addStep">+ 添加</button></div>
        <div v-for="(step, idx) in processSteps" :key="idx" class="step-row">
          <span class="step-row__num">{{ idx + 1 }}</span>
          <textarea v-model="step.content" class="field-textarea" rows="2" placeholder="描述这一步…" />
